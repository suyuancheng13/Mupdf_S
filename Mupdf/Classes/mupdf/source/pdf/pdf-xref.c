...
pdf_read_ocg(pdf_document *doc)
{
	...
	if (!obj)
		return;
	ocg = pdf_dict_gets(obj, "OCGs");//get the array of the ocgs
	...
	pdf_ocg_set_config(doc, 0);//first time to set config
}
...
pdf_init_document(pdf_document *doc)
{
	...
	fz_try(ctx)
	{
		pdf_read_ocg(doc);//read the optional content group
