...
{
    pdf_doc_event base;
    pdf_submit_event submit;
}pdf_submit_event_internal;

pdf_submit_event *pdf_access_submit_event(pdf_doc_event *event)
{
    pdf_submit_event *submit = NULL;
    if (event->type == PDF_DOCUMENT_EVENT_SUBMIT)
        submit = &((pdf_submit_event_internal *)event)->submit;
    return submit;
}

void pdf_event_issue_submit(pdf_document *doc, char *url, char *data, int get, int data_len)
{
    if (doc->event_cb)
    {
        pdf_submit_event_internal e;
        
        e.base.type = PDF_DOCUMENT_EVENT_SUBMIT;
        e.base.doc = doc;
        e.submit.url = url;
        e.submit.data = data;
        e.submit.data_len = data_len;
        e.submit.get = get;
