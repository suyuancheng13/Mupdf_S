...
pdf_rebuild_page_tree(pdf_document *doc)
{
	pdf_obj *catalog;
	pdf_obj *pages;
    pdf_obj *catalog_o;// add by suyuancheng
    ...
    /*
     add by suyuancheng
     */
    catalog_o = pdf_load_object(doc, pdf_to_num(catalog), 0);
	pdf_dict_puts_drop(catalog_o, "Pages", pages);
    pdf_print_obj(catalog);
    pdf_update_object(doc, pdf_to_num(catalog), catalog_o);
