...
pdf_jsimp_obj *pdf_jsimp_new_array_obj(pdf_jsimp *imp, pdf_jsimp_type *type, pdf_jsimp_obj *natobj[2])
{
	pdf_jsimp_obj *obj = NULL;
	const char *err = pdf_jsimp_new_array_obj_cpp(imp, type, natobj, &obj);
	if (err != NULL)
		fz_throw(pdf_jsimp_ctx_cpp(imp), FZ_ERROR_GENERIC, "%s", err);
...
// pdf_jsimp_obj* pdf_jsimp_from_array(pdf_jsimp *imp,void *array)
//{
//    pdf_jsimp_obj *obj = NULL;
//    const char *err = pdf_jsimp_from_array_cpp(imp, array, &obj);
//    if(err == NULL)
//        fz_throw(pdf_jsimp_ctx_cpp(imp), FZ_ERROR_GENERIC, "%s", err);
//    
//	return obj;
...
int  pdf_jsimp_to_boolean(pdf_jsimp *imp, pdf_jsimp_obj *obj)
{
	int boo;
	const char *err = pdf_jsimp_to_boolean_cpp(imp, obj, &boo);
	if (err != NULL)
		fz_throw(pdf_jsimp_ctx_cpp(imp), FZ_ERROR_GENERIC, "%s", err);
