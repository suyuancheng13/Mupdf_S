...
pdf_new_crypt(fz_context *ctx, pdf_obj *dict, pdf_obj *id)
{
	...
	if (strcmp(pdf_to_name(obj), "Standard") != 0)//standar fileter
	{
		pdf_free_crypt(ctx, crypt);
	...
	crypt->v = 0;
	obj = pdf_dict_gets(dict, "V");//represent which algorithm to use
	if (pdf_is_int(obj))
	...
	obj = pdf_dict_gets(dict, "R");//the rivision of the algorithm
	if (pdf_is_int(obj))
	...
	obj = pdf_dict_gets(dict, "P");//the permission to do which operation 
	if (pdf_is_int(obj))
...
pdf_compute_encryption_key(pdf_crypt *crypt, unsigned char *password, int pwlen, unsigned char *key)
{
	...
	unsigned int p;
	int i, n;
	fz_md5 md5;// the hash function md5
...
pdf_authenticate_user_password(fz_context *ctx, pdf_crypt *crypt, unsigned char *password, int pwlen)
{
	...
	pdf_compute_user_password(ctx, crypt, password, pwlen, output);//password -> output
...
pdf_authenticate_owner_password(fz_context *ctx, pdf_crypt *crypt, unsigned char *ownerpass, int pwlen)
{
	...
	/* Step 2 (Revision 3 or greater) decrypt the owner password*/
	if (crypt->r >= 3)
	{
		...
		for (x = 0; x < 20; x++)
		{
			for (i = 0; i < 32; i++)
				xor[i] = pwbuf[i] ^ (19 - x);
			fz_arc4_init(&arc4, xor, n);
			fz_arc4_encrypt(&arc4, userpass, userpass, 32);//only encrypt the password
		}
	}
...
/*
 TODO:create  a owner password to store in O entry 
 */
static
unsigned char * pdf_compute_owner_password(fz_context *ctx, pdf_crypt *crypt, unsigned char *ownerPwd,int opwlen,unsigned char *userPwd,int upwlen )
{
    unsigned char pwbuf[32];
	...
	/* Step a  */
	/* copy and pad owner password string */
	if (opwlen > 32)
		opwlen = 32;
	memcpy(pwbuf, ownerPwd, opwlen);
	memcpy(pwbuf + opwlen, padding, 32 - opwlen);
    
	/* step b: take md5 hash of padded password */
	fz_md5_init(&md5);
	fz_md5_update(&md5, pwbuf, 32);
	fz_md5_final(&md5, key);
    
    /* step c:do some voodoo 50 times (Revision 3 or greater) */
	if (crypt->r >= 3)
	{
		for (i = 0; i < 50; i++)
		{
    ...
    /* step e:copy and pad user password string */
	if (upwlen > 32)
		upwlen = 32;
	memcpy(ubuf, userPwd, upwlen);
	memcpy(ubuf + upwlen, padding, 32 - upwlen);
    memcpy(userpass, ubuf ,32);
	...
	if (crypt->r >= 3)
	{
		...
		for (x = 0; x <= 19; x++)
		{
			for (i = 0; i < 32; i++)
				xor[i] = pwbuf[i] ^(19- x);
			fz_arc4_init(&arc4, xor, n);//create the encrypt key
			fz_arc4_encrypt(&arc4, userpass, userpass, 32);//only encrypt the password
		}

	}
    memcpy(crypt->o, userpass, 32);
...
/*
 *TODO:create a user password 
 */
void pdf_create_user_password(fz_context *ctx, pdf_crypt *crypt, const char *newpassword, int pwlen,unsigned char *newUserPwd)
{
    char password[2048];
    password[0] = 0;
    if (newpassword)
    {
        if (crypt->r <= 4)
            pdf_docenc_from_utf8(password, newpassword, sizeof password);
        else
            pdf_saslprep_from_utf8(password, newpassword, sizeof password);
    }
    pdf_compute_user_password(ctx, crypt ,(unsigned char *)password , strlen(password),newUserPwd );
}
/*
 * authenrized the password and also keep the key that to decrypt the strings and streams in the crypr object
 */
...
unsigned char * pdf_create_owner_password(pdf_document *doc,const char * ownerpassword,const char *userpassword,unsigned char *output)
{
    char owner[2048];
    char user[2048];
    ...
    if (ownerpassword && userpassword)
    {
        if (doc->crypt->r <= 4)
        {
            pdf_docenc_from_utf8(owner, ownerpassword, sizeof owner);
            pdf_docenc_from_utf8(user, userpassword, sizeof user);
        }
        else
        {
            pdf_saslprep_from_utf8(owner, ownerpassword, sizeof owner);
            pdf_saslprep_from_utf8(user, userpassword, sizeof user);
        }
    }
    pdf_compute_owner_password(doc->ctx, doc->crypt, (unsigned char *)owner, strlen(owner), (unsigned char*)user, strlen(user));
    return NULL;
}
